...
_jwks_cache = ThreadSafeTTLCache[str, dict[str, Any]](max_entries=128, clock=lambda: time.time())
_jwks_backoff_until = ThreadSafeTTLCache[str, float](max_entries=128, clock=lambda: time.time())
...
def _get_jwks(settings: Settings) -> dict[str, Any]:
    ...
    with _jwks_lock:
        ...
        if cached is not None:
            ...
            return cached
        blocked_until = _jwks_backoff_until.get(settings.oidc_jwks_url) or 0.0
    ...
    try:
        ...
        with _jwks_lock:
            _jwks_cache.set(
                settings.oidc_jwks_url, payload, ttl_seconds=settings.jwks_cache_ttl_seconds
            )
            _jwks_backoff_until.delete(settings.oidc_jwks_url)
        record_jwks_fetch(outcome="success")
        return payload
    except Exception as exc:
        with _jwks_lock:
            previous = _jwks_backoff_until.get(settings.oidc_jwks_url) or now
            next_backoff = min(
                max(1.0, (previous - now) * 2.0 if previous > now else 1.0),
                float(settings.jwks_backoff_max_seconds),
            )
            _jwks_backoff_until.set(
                settings.oidc_jwks_url,
                now + next_backoff,
                ttl_seconds=max(next_backoff, float(settings.jwks_backoff_max_seconds)),
            )
